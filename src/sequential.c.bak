...
int main(int argc, char **argv)
{
    ...
    // Convert pixels into grayscale (all frames)
    apply_gray_filter(image);
    // Apply blur filter (3x3, single pass)
    apply_blur_filter(image, 3, 0);
    // Apply Sobel filter (all frames)
    apply_sobel_filter(image);

    gettimeofday(&t2, NULL);
    duration = (t2.tv_sec - t1.tv_sec) + ((t2.tv_usec - t1.tv_usec) / 1e6);

    // IMPORTANT: This single line is printed after processing all frames
    // The time represents the total for all frames of the GIF.
