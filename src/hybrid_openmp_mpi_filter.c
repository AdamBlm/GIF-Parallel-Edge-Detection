...
static void apply_gray_filter(pixel **p, int n_images, int *widths, int *heights)
{
...
static void apply_blur_filter_to_image(pixel *p, int width, int height, int size, int threshold, pixel *buffer)
{
    int end = 0;
    int n_iter = 0;
...
static void apply_blur_filter(pixel **p, int n_images, int *widths, int *heights, int size, int threshold, pixel **buffers)
{
...
static void apply_sobel_filter_to_image(pixel *p, int width, int height, pixel *buffer)
{
...
static void apply_sobel_filter(pixel **p, int n_images, int *widths, int *heights, pixel **buffers)
{
...
static void free_resources(pixel **p, int n_images)
{
    if (p) {
        for (int i = 0; i < n_images; i++) {
            if (p[i]) free(p[i]);
...
scatter_info* create_scatter_info_mpi(int n_images, int size)
{
...
void free_scatter_info_mpi(scatter_info *info)
{
    if (info) {
        if (info->sendcounts) free(info->sendcounts);
        if (info->displs) free(info->displs);
...
void calculate_pixel_counts_mpi(scatter_info* scatter_data, int* widths, int* heights, int size) {
...
/* Applies Sobel filter to multiple images */
static void apply_sobel_filter_multiple(pixel **p, int n_images, int *widths, int *heights, pixel **buffers) {
    #pragma omp parallel for default(none) shared(p, n_images, widths, heights, buffers) schedule(dynamic)
    for (int i = 0; i < n_images; i++) {
        apply_sobel_filter_to_image(p[i], widths[i], heights[i], buffers[i]);
    }
...
/* ===================== Public Hybrid Filtering Function ===================== */
 ...
 * @num_threads: Number of OpenMP threads to use (if <= 0, default is used).
 *
...
{
       int rank, size;
    // Initialize MPI with NULL arguments.
    ...
    if (rank == 0)
    {
        ...
        scatter_data = create_scatter_info_mpi(n_images, size);
        if (!scatter_data) {
        ...
        calculate_pixel_counts_mpi(scatter_data, image_widths, image_heights, size);
    }
    ...
    free_scatter_info_mpi(scatter_data);
