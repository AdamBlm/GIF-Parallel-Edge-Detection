...
apply_gray_filter(animated_gif * image)
{
    ...
    #pragma omp parallel for default(none) shared(image, p) private(i, j)
    for (i = 0; i < image->n_images; i++)
    {
        for (j = 0; j < image->width[i] * image->height[i]; j++)
        {
...
apply_blur_filter(animated_gif * image, int size, int threshold)
{
    ...
    /* Process all images */
    #pragma omp parallel for default(none) shared(image, p, size, threshold) private(j, k, width, height, end, n_iter, new)
    for (i = 0; i < image->n_images; i++)
    {
        n_iter = 0;
        width = image->width[i];
        ...
        do
        {
            ...
            /* Initialize the new pixels array with original values */
            for(j=0; j<height; j++)
            {
                for(k=0; k<width; k++)
                {
...
apply_sobel_filter(animated_gif * image)
{
    ...
    #pragma omp parallel for default(none) shared(image, p) private(j, k, width, height)
    for (i = 0; i < image->n_images; i++)
    {
        width = image->width[i];
        height = image->height[i];
        ...
        /* IMPORTANT FIX: Initialize the entire sobel buffer to zeros */
        for(j = 0; j < height; j++) {
            for(k = 0; k < width; k++) {
        ...
        /* Apply Sobel filter to inner pixels */
        for(j=1; j<height-1; j++)
        {
            for(k=1; k<width-1; k++)
            {
                ...
                /* IMPORTANT CHANGE: Lower threshold or use dynamic thresholding */
                /* If your image is still mostly black, try reducing this threshold value */
                if (val_blue > 30) /* Changed from 50 to 30 */
                {
                    sobel[CONV(j,k,width)].r = 255;
                    sobel[CONV(j,k,width)].g = 255;
                    sobel[CONV(j,k,width)].b = 255;
                }
                /* else values stay 0 from initialization */
            }
        }
